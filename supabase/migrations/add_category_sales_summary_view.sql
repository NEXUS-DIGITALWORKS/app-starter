...
-- カテゴリ単位に集計した直近2年の売上合計。product_list_with_sales の sales_total_2y
...
-- 商品全体の売上合計とは一致しない）。
--
-- DROP VIEW不要のCREATE OR REPLACE VIEWのため再実行安全。ただし
-- add_product_list_with_sales_view.sql を再実行するとCASCADEでこのビューも
-- 一緒に削除されるため、その直後には必ずこのファイルを再実行して作り直すこと。
-- =========================================================

CREATE OR REPLACE VIEW public.category_sales_summary AS
SELECT
  pc.category_id,
  COALESCE(SUM(pls.sales_total_2y), 0)::NUMERIC AS sales_total_2y
