...
const TOOL_ENTRIES: ToolEntry[] = [
  ...
  },
  {
    id: 'tech-guide',
    title: 'テックガイド',
    desc: '主要な技術要素の特徴と向いているケースを解説します。',
    href: '/tools/tech-guide',
    icon: BookOpen,
  },
  {
    id: 'risk-check',
    title: 'リスクチェック55',
    desc: '8分類・55項目でシステムのリスクを洗い出します。',
    href: '/tools/risk-check',
    icon: ShieldCheck,
  },
];

const RISK_CHECK_CATEGORIES = [
  '①外部依存',
  '②セキュリティ',
  '③データ',
  '④AI・開発品質',
...
export default function ToolsHome() {
  ...
  return (
    <div className="page tools-home-page tools-scope">
      ...
      <main className="home-main">
        ...
        <section className="tools-grid-section">
          <div className="tools-grid-head">
            <h2>4つのツールで、アイデアから技術構成まで整理する。</h2>
            <p>目的に合わせて、好きなツールから始められます。</p>
          </div>
          <div className="tools-grid">
            {TOOL_ENTRIES.map((tool) => {
              const Icon = tool.icon;
              return (
                <Link className="tool-card" to={tool.href} key={tool.id}>
        ...
        <section className="risk-teaser">
          <div className="risk-teaser-text">
            <h2>作ったシステムのリスクも、8分類でチェックできます。</h2>
            <p>
              外部依存・セキュリティ・データ・AI開発品質・インフラ・コスト・運用・事業継続の8分類、計55項目でリスクを可視化。
              各項目は「完全自動」「AI半自動」「手動」いずれかの判定方式に整理されているので、何から着手すべきかが一目でわかります。
            </p>
            <Link className="risk-teaser-btn" to="/tools/risk-check">
              リスクチェック55を見る →
            </Link>
          </div>
          <ul className="risk-teaser-categories">
            {RISK_CHECK_CATEGORIES.map((category) => (
