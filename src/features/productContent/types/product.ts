...
export interface ProductListItem {
  ...
  salesTotal2y: number; // 直近2年の累積売上（product_list_with_sales ビューの sales_total_2y）
}

export type UpdatedWithinFilter = 'all' | 'today' | '7d' | '30d';

// 商品一覧の並び順。sales_total_2y は product_list_with_sales ビュー
// （add_product_list_with_sales_view.sql参照）が持つ集計列。
export type ProductSortBy = 'updated_at' | 'sales_total_2y';
