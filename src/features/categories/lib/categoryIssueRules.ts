...
// 「直近1年の売上が非常に低い」と判断する金額の閾値（円、この金額未満が対象）。
// 明確な基準がないため暫定値。運用しながら調整する想定。
export const LOW_SALES_1Y_MAX = 10000;
...
export function evaluateCategoryIssues(row: CategoryIssueRow): CategoryIssueType[] {
  ...
  if (row.productCount > 0 && row.salesTotal1y < LOW_SALES_1Y_MAX) issues.push('low_sales_1y');
