...
import { AlertTriangle, ArrowDown, ArrowRight, BarChart3, Check, FileText, Layers3, Rocket, Target } from 'lucide-react';
import { Link } from 'react-router-dom';
import { cn } from '../../../lib/utils';
import { getCategories } from '../data/categories';
import { CATEGORY_ICONS, DEFAULT_CATEGORY_ICON } from '../lib/categoryIcons';
import type { Selection, TechElement } from '../types';
...
type LayerConfig = {
  ...
  supportingCategoryIds?: string[];
  emptyNote: string;
  highlights: string[];
  filledHighlights?: string[];
};
...
const LAYERS: LayerConfig[] = [
  {
    ...
    primaryCategoryIds: ['mobile-desktop'],
    emptyNote: 'ユーザー / ブラウザ',
    highlights: ['Webブラウザでアクセス', 'マルチデバイス対応'],
    filledHighlights: ['マルチデバイス対応', 'ネイティブ機能へのアクセス'],
  },
  {
    key: 'frontend',
    name: 'FRONTEND',
    ...
    supportingCategoryIds: ['ui-design'],
    emptyNote: '専用のフロントエンド技術は選択されていません',
  ...
  },
  {
    ...
    primaryCategoryIds: ['database'],
    supportingCategoryIds: ['orm'],
    emptyNote: '専用のデータベース技術は選択されていません',
...
const CROSS_CUTTING: { categoryId: string; label: string; elementIds?: string[] }[] = [
  { categoryId: 'auth', label: '認証・認可' },
  { categoryId: 'storage', label: 'ストレージ' },
  { categoryId: 'ai', label: 'AI・自動化連携' },
  { categoryId: 'hosting', label: '公開・実行基盤' },
  { categoryId: 'dev-approach', label: 'CI/CD・監視', elementIds: ['github-actions', 'sentry'] },
];

function resolveElements(selection: Selection, categoryId: string, onlyIds?: string[]) {
  const category = getCategories().find((c) => c.id === categoryId);
  if (!category) return { category: null, elements: [] as TechElement[] };
  const ids = selection[categoryId] ?? [];
  const filteredIds = onlyIds ? ids.filter((id) => onlyIds.includes(id)) : ids;
  const elements = filteredIds
    .map((id) => category.elements.find((e) => e.id === id))
    .filter((e): e is TechElement => Boolean(e));
  return { category, elements };
}

type LayerItem = { element: TechElement; categoryId: string; isPrimary: boolean };

function resolveLayerItems(layer: LayerConfig, selection: Selection): LayerItem[] {
  const groups = [
    ...layer.primaryCategoryIds.map((id) => ({ id, isPrimary: true })),
    ...(layer.supportingCategoryIds ?? []).map((id) => ({ id, isPrimary: false })),
  ];
  return groups.flatMap(({ id, isPrimary }) => {
    const { category, elements } = resolveElements(selection, id);
    if (!category) return [];
    return elements.map((element) => ({ element, categoryId: category.id, isPrimary }));
  });
}
...
function FlowRow({
  ...
  const items = resolveLayerItems(layer, selection);
  const primaryElements = items.filter((i) => i.isPrimary);
  const theme = LAYER_THEME[themeIndex];
  const Icon: LucideIcon = primaryElements[0]
    ? CATEGORY_ICONS[primaryElements[0].categoryId] ?? DEFAULT_CATEGORY_ICON
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-[14.5px] font-bold leading-snug text-foreground">
                  {primaryElements.map((p) => p.element.name).join(' / ')}
                </p>
                <ul className="mt-1.5 flex flex-col gap-0.5">
                  {(layer.filledHighlights ?? layer.highlights).map((h) => (
...
function TechPanel({ selection }: { selection: Selection }) {
  const crossItems = CROSS_CUTTING.map((def) => {
    const { elements } = resolveElements(selection, def.categoryId, def.elementIds);
    return elements;
  }).flat();
          ...
          return (
            <div key={layer.key}>
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {items.map(({ element, categoryId }) => {
                  const Icon = CATEGORY_ICONS[categoryId] ?? DEFAULT_CATEGORY_ICON;
                  return (
                    <div
                      key={element.id}
                    ...
                    >
                      ...
                      <span className="text-[10.5px] font-semibold leading-snug text-foreground">{element.name}</span>
                    </div>
                  );
                })}
              </div>
...
export default function PatternArchitectureDiagram({
  selection,
  purpose,
  merits,
  notes,
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[2.1fr_1fr_1fr]">
        ...
        <TechPanel selection={selection} />
        <InfoCards purpose={purpose} merits={merits} notes={notes} cautions={cautions} />
      </div>

      <Link
        to={ctaHref}
        className="flex items-center justify-center gap-2 rounded-2xl border border-primary/20 bg-primary/5 px-4 py-3 text-center hover:bg-primary/10"
      >
